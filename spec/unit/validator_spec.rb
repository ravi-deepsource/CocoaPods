  ...
  describe Validator do
    ...
    describe "Quick mode" do
      ...
      it "lints the podspec during validation" do
        ...
        validator.quick = true
        validator.validate
        validator.results.map(&:to_s).first.should =~ /should match the name/
      ...
      it "respects the allow warnings option" do
        ...
        validator.validate
        validator.results.map(&:to_s).first.should =~ /summary.*meaningful/
        validator.validated?.should.be.true
      end
    ...
    describe "Extensive analysis" do
      ...
      describe "Platforms" do
        ...
        it "includes only tests supported on the current platform" do
          ...
          subspec = Specification.new(validator.spec, "Tests", true) { |s|
            s.platform = :ios
          }
      ...
      it "test_pod runs multiple test_specs" do
        ...
        installer = stub(pod_targets: [pod_target_one],
                         target_installation_results: [pod_target_installation_results])
        validator.instance_variable_set(:@installer, installer)
        test_spec1 = Specification.new(validator.spec, "Tests1", true) { |s|
          s.platform = :ios
        }
        test_spec2 = Specification.new(validator.spec, "Tests2", true) { |s|
          s.platform = :ios
        }
        test_spec3 = Specification.new(validator.spec, "Tests3", true) { |s|
          s.platform = :ios
        }
      ...
      it "uses the deployment target of the current subspec" do
        ...
        Installer.any_instance.stubs(:aggregate_targets).returns([])
        Installer.any_instance.stubs(:pod_targets).returns([])
        subspec = Specification.new(validator.spec, "subspec") { |s|
      ...
      it "uses the deployment target of the current test spec" do
        ...
        test_spec = Specification.new(validator.spec, "testspec", true) { |s|
      ...
      describe "import validation" do
        ...
        it "adds developer framework paths when the pod depends on XCTest" do
          ...
          app_project.native_targets.first.build_configurations.map { |bc|
            bc.build_settings["FRAMEWORK_SEARCH_PATHS"]
          }.uniq.should == [%w($(inherited) "$(PLATFORM_DIR)/Developer/Library/Frameworks")]
        end
        ...
        it "adds developer library paths when the pod depends on XCTest" do
          ...
          }.uniq.should == [%w($(inherited) "$(PLATFORM_DIR)/Developer/usr/lib")]
        end
      ...
      describe "file pattern validation" do
        it "checks for file patterns" do
          ...
          validator.stubs(:validate_url)
          validator.validate
          validator.results.map(&:to_s).first.should =~ /source_files.*did not match/
          validator.result_type.should == :error
        end
        ...
        it "checks private_header_files matches only headers" do
          ...
          validator.stubs(:validate_url)
          validator.validate
          validator.results.map(&:to_s).first.should =~ /matches non-header files \(JSONKit\.m\)/
          validator.result_type.should == :error
        end
        ...
        it "warns if public_header_files does not match any files" do
          ...
          validator.stubs(:validate_url)
          validator.validate
          validator.results.map(&:to_s).first.should =~ /The `public_header_files` pattern did not match any file./
          validator.result_type.should == :warning
        end
        ...
        it "warns if private_header_files does not match any files" do
          ...
          validator.stubs(:validate_url)
          validator.validate
          validator.results.map(&:to_s).first.should =~ /The `private_header_files` pattern did not match any file./
          validator.result_type.should == :warning
        end
        ...
        it "checks presence of license file" do
          ...
          validator.stubs(:validate_url)
          validator.validate
          validator.results.map(&:to_s).first.should =~ /Unable to find a license file/
          validator.result_type.should == :warning
        end
        ...
        it "checks module_map must exist if specified" do
          ...
          validator.stubs(:validate_url)
          validator.validate
          validator.results.map(&:to_s).first.should =~ /Unable to find the specified module map file./
          validator.result_type.should == :error
        end
        ...
        it "checks module_map accepts only modulemaps" do
          ...
          validator.stubs(:validate_url)
          validator.validate
          validator.results.map(&:to_s).first.should =~ /Unexpected file extension for modulemap file \(JSONKit\.m\)/
          validator.result_type.should == :error
        end
    ...
    describe "dynamic binaries validation" do
      it "fails with dynamic binaries on iOS < 8" do
        ...
        validator.results.map(&:to_s).first.should =~ /Dynamic frameworks.*iOS 8.0 and onwards/
        validator.result_type.should == :error
      end
    end
    ...
    describe "swift validation" do
      ...
      it "fails on deployment target < iOS 8 for Swift Pods" do
        ...
        validator.results.map(&:to_s).first.should =~ /dynamic frameworks.*iOS > 8/
        validator.result_type.should == :error
      end
