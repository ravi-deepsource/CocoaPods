  ...
  describe Installer::Analyzer do # rubocop:disable Metrics/BlockLength
    describe "Analysis" do # rubocop:disable Metrics/BlockLength
      ...
      it "does not update non-updateable repositories" do
        ...
        UI.output.should =~ /Skipping `#{source.name}` update because the repository is not an updateable repository./
      ...
      describe "dependent pod targets" do
        it "raises when a pod depends on a non-library spec" do
          ...
          pod_a = Pod::Spec.new { |s|
            s.name = "a"
            s.version = "1.0"
            s.test_spec "Tests"
            s.app_spec "App"
          }
          pod_b = Pod::Spec.new { |s|
            s.name = "b"
            s.version = "1.0"
      ...
      it "raises if change in local pod specification conflicts with lockfile" do
        ...
        }.message.should.match(/You should run `pod update Expecta`/)
      end
      ...
      it "raises if dependencies need to be fetched but fetching is not allowed" do
        ...
        analyzer = Installer::Analyzer.new(sandbox, podfile, lockfile)
        error = should.raise(Informative) {
          analyzer.analyze(false)
        }
      ...
      it "warns when a dependency is duplicated" do
        ...
        UI.warnings.should =~ /duplicate dependencies on `RestKit`/
        UI.warnings.should =~ /RestKit \(~> 0.23.0\)/
      ...
      describe "#filter_pod_targets_for_target_definition" do
        ...
        it "raises if a Pod is whitelisted for different build configurations" do
          ...
          should.raise(Informative) {
            @analyzer.analyze
          }.message.should.include "The subspecs of `AFNetworking` are linked to different build configurations for the `Pods-SampleProject` target. CocoaPods does not currently support subspecs across different build configurations."
        end
      end
      ...
      describe "extension targets" do
        ...
        it "does not copy a static library's pod target, when the static library aggregate target has search paths inherited" do
          ...
          result.targets.flat_map { |aggregate_target|
    ...
    describe "Analysis, concerning naming" do
      ...
      it "raises when dependencies with the same name have different " \
        ...
        e.message.should =~ /different sources for `SEGModules`/
        e.message.should =~ %r{SEGModules \(from `https://github.com/segiddins/SEGModules.git`\)}
      ...
      it "raises when dependencies with the same root name have different " \
        ...
        e.message.should =~ /different sources for `RestKit`/
        e.message.should =~ %r{RestKit/Core \(from `https://github.com/RestKit/RestKit.git`\)}
      ...
      it "raises when dependencies with the same name have different " \
        ...
        e.message.should =~ /different sources for `RestKit`/
        e.message.should =~ %r{RestKit \(from `https://github.com/RestKit/RestKit.git`\)}
